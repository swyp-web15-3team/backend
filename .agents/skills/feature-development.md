...
description: Plan and implement minimal backend features with a 400-line limit only for existing-feature modifications; creation and deletion are unlimited. Use for new features, feature extensions, continuing a plan, or splitting an oversized feature into follow-up PRs. Exclude standalone reviews and explanatory questions.
---
...
- PR breakdown: classify as feature creation, modification, or deletion; for modifications, estimate added plus deleted lines, including tests, docs, and configuration.
- Budget: for modifications, leave room below 400 lines for regression tests and review fixes.
...
- Applicability: only existing-feature modification PRs have a line limit. Feature creation and deletion PRs are exempt. Classify by behavior, not Git file status; mixed PRs that modify existing features use `modify`.
- Threshold: for modifications, added lines + deleted lines must be **at most 400**; 400 passes, 401 fails.
- Coverage: for modifications, count every changed text file, including tests, configuration, docs, generated files, and lockfiles.
...
- Binary changes: for modifications, the line check cannot certify them; stop for an explicit review policy instead of counting them as zero.
...
- Command: `bash .agents/scripts/check-pr-size.sh <actual-base-ref> <candidate-head-ref> <create|modify|delete>`.
- Timing: pass the classified change type; run before opening/updating each PR and again after review fixes or base changes.
...
- Completion: report delivered behavior, checks, modification line counts or creation/deletion exemptions, and any remaining slices or blockers.
...
| Feature modification exceeds 400 changed lines | Stop publication; split and rerun the size check |
...
- Normal: plan a small endpoint modification, implement and test it, pass the line gate, then obtain an independent review.
- Boundary: for modifications, 400 added/deleted lines pass; 401 fail.
- Exemptions: feature creation and deletion pass regardless of line count.
- Overflow: split a larger feature modification into valid PRs, including tests in each slice's budget.
- Error: an unknown change type, unknown base, or modification binary diff fails the gate rather than reporting a misleading count.
